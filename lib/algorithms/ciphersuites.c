 ...
 * Draft support for verify_data_length entries in cipher suite
 * specifications.
 * 
 * Also check gnutls_int.h for updatet struct definition.
 * Also check gnutls_handshake.c for altered behavior.
 * 
 * Note: verify_data_length value may not exceed MAX_PRF_BYTES value
 * 			 defined in gnutls_state.c. The latter is currently set to 200.
 */

/* Cipher SUITES */
#define ENTRY( name, block_algorithm, kx_algorithm, mac_algorithm, min_version, dtls_version ) \
	{ #name, name, block_algorithm, kx_algorithm, mac_algorithm, min_version, dtls_version, GNUTLS_MAC_SHA256, NONCE_IS_SENT, VERIFY_DATA_LEN_UNKNOWN}
	/* REMARK:
	 * Maybe useful to rename this macro? As it now also facilitates in 
	 * specifying the verify_data_length property instead of only 
	 * specifying a PRF.
	 */ 
#define ENTRY_PRF( name, block_algorithm, kx_algorithm, mac_algorithm, min_version, dtls_version, prf, verify_data_length ) \
	{ #name, name, block_algorithm, kx_algorithm, mac_algorithm, min_version, dtls_version, prf, NONCE_IS_SENT, verify_data_length}
#define ENTRY_CNONCE( name, block_algorithm, kx_algorithm, mac_algorithm, min_version, dtls_version, verify_data_length ) \
	{ #name, name, block_algorithm, kx_algorithm, mac_algorithm, min_version, dtls_version, GNUTLS_MAC_SHA256, NONCE_IS_COUNTER, verify_data_length}
...
/* ARPA2 added by TomV for TLS-KDH: */
/* ECC with KDH; draft-vanrein-tls-kdh-04 */
#define GNUTLS_ECDHE_KDH_AES_128_GCM_SHA256 {0xFF, 0xA1}
#define GNUTLS_ECDHE_KDH_AES_256_GCM_SHA384 {0xFF, 0xA2}
#define GNUTLS_ECDHE_KDH_AES_256_GCM_SHA512 {0xFF, 0xA3}
#define GNUTLS_ECDHE_KDH_AES_128_CCM {0xFF, 0xA4}
...
#define GNUTLS_ECDHE_KDH_CAMELLIA_128_GCM_SHA256 {0xFF, 0xA8}
#define GNUTLS_ECDHE_KDH_CAMELLIA_256_GCM_SHA384 {0xFF, 0xA9}
#define GNUTLS_ECDHE_KDH_CAMELLIA_256_GCM_SHA512 {0xFF, 0xAA}
/* end */
...
/* GCM */
	...
	ENTRY_CNONCE(GNUTLS_RSA_CHACHA20_POLY1305,
		...
		GNUTLS_MAC_AEAD, GNUTLS_TLS1_2,
		GNUTLS_DTLS1_2, VERIFY_DATA_LEN_UNKNOWN),
...
/* ARPA2 added by TomV for TLS-KDH:
 * 
 * According to https://tools.ietf.org/html/rfc5246#section-5
 * all new ciphersuites should specify a PRF. Hence the use of
 * ENRTY_PRF. 
 */
...
uint16_t _gnutls_cipher_suite_get_verify_data_len(const uint8_t suite[2])
{
	...
	CIPHER_SUITE_ALG_LOOP(ret = p->verify_data_length, suite);
	return ret;
}
/* end */
...
{
	unsigned i;
	static const unsigned NUM_SUITES = 10;
	static const uint8_t KDH_suites[10][2] = {
	...
	// Find a matching suite in the above list
