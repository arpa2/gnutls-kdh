...
static int _gnutls_server_cert_type_pack( extension_priv_data_t epriv, 
										gnutls_buffer_st* ps );
static int _gnutls_server_cert_type_unpack( gnutls_buffer_st* ps, 
										extension_priv_data_t* epriv );
static void _gnutls_server_cert_type_deinit( extension_priv_data_t priv );

inline static int _gnutls_num2cert_type( int num );
inline static int _gnutls_cert_type2num( int cert_type );

extension_entry_st ext_mod_server_cert_type = {
	.name = "Server Certificate Type",
	.type = GNUTLS_EXTENSION_SERVER_CERT_TYPE,
	.parse_type = GNUTLS_EXT_TLS,
	.recv_func = _gnutls_server_cert_type_recv_params,
	.send_func = _gnutls_server_cert_type_send_params,
	.pack_func = _gnutls_server_cert_type_pack,
	.unpack_func = _gnutls_server_cert_type_unpack,
	.deinit_func = _gnutls_server_cert_type_deinit
};
...
{	
	...
	} else // server mode
	{
		...
		epriv = cert_types;
		_gnutls_ext_set_session_data( session, GNUTLS_EXTENSION_SERVER_CERT_TYPE, epriv );
		
		/* We reveive a list of supported certificate types that the client
										...
										gnutls_buffer_st* data )
{
	...
	{ 
		// For brevity
		cert_priors = &session->internals.priorities.server_cert_type;
		...
		{ 
			 ...
			 * credentials (they are not needed) and only check the 
			...
			for( i = 0; i < cert_priors->algorithms; i++ ) 
			{
								...
								GNUTLS_CTYPE_SERVER ) == 0 ) 
				{
					...
					_gnutls_handshake_log
					("EXT[%p]: Sent server cert type (%d) %s\n", session,
	...
	} else // Server mode
	{
		...
		cert_type = _gnutls_cert_type2num( session->security_parameters.server_cert_type );
